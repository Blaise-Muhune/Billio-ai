  ...
  .max-w-7xl.mx-auto(class="px-4 sm:px-6 lg:px-8 py-12")
    ...
    .grid(class="grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto")
      ...
      .bg-white.rounded-2xl.shadow-lg.transition-all.duration-300(
        ...
        .mt-8.space-y-4
          .flex.items-center.gap-3(v-for="feature in ['Up to 20 business cards', 'Advanced card scanning', '10 email drafts per card','no banner ads', 'Up to 5 events per month', 'Enhanced digital profile', 'QR code integration', 'Download QR codes', 'Full social media integration', 'AI email generation']" :key="feature")
            VaIcon(name="check_circle" size="20px" class="text-emerald-500")
            span.text-gray-700 {{ feature }}
